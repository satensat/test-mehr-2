...
export default function MilitaryStatusList({ formik }) {
  ...
  const handleChangeInputAutoComplete = (event) => {
    ...
    const filteredList = listItemsSource.filter((militaryStatus) =>
      militaryStatus.name.includes(event.target.value)
    );
  ...
  const handleEnterKeyPress = (event) => {
    ...
    setFilteredList(filteredList);
    if (event.key === "Enter") {
      formik.setFieldValue("militaryStatus", filteredList[0].name);
    ...
    <div className="flex flex-col grow w-full">
              ...
              {filteredList.map((item, index) => {
                return (
                  <button
                    ...
                    name={item.name}
                    onClick={() => {
                      formik.setFieldValue("militaryStatus", item.name);
      ...
      <div
        className={
          ...
          " " +
          `${
            formik.errors.militaryStatus && formik.touched.militaryStatus
              ? // &&
                // formik.errors.militaryStatus.name &&
                // formik.touched.militaryStatus.name
                " opacity-100 "
              : " opacity-0 "
          }`
        }
