...
//               ? ""
//               : new Date(formik.values.start_date).toLocaleDateString("fa-IR")
//           }
//           readOnly
...
export default function StartDatePicker({ formik }) {
  ...
  const handleCloseList = () => {
    // formik.setFieldValue("start_date", filteredList[0].name);
  ...
  useEffect(() => {
      ...
      setTextInput(formik.values.start_date );

  }, [formik.values.start_date]);
  const handleChangeInputAutoComplete = (event) => {
    // console.log(event);
    setTextInput(() => event.target.value);
    const filteredList = listItemsSource.filter((start_date) =>
      start_date.includes(event.target.value)
    );
  ...
  const handleEnterKeyPress = (event) => {
    ...
    setFilteredList(filteredList);
    if (event.key === "Enter") {
      formik.setFieldValue("start_date", filteredList[0]);
              ...
              {filteredList.map((item, index) => {
                return (
                  <button
                    ...
                    name={item}
                    onClick={() => {
                      formik.setFieldValue("start_date", item);
                      setTextInput(item);
                      setStatusList(false);
                    }}
      ...
      <div
        className={
          ...
          `${
            formik.errors.start_date && formik.touched.start_date
              ? // &&
                // formik.errors.start_date.name &&
                // formik.touched.start_date.name
                " opacity-100 "
              : " opacity-0 "
          }`
        }
