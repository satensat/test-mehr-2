...
export default function CertificateComponent({ formik }) {
  const openFilePickerPicture = () => {
    document.getElementById("fileInputPictureCertificate").click();
  };
  ...
  const handleFileDeleteFromListPicture = (fileClicked, indexClicked) => {
    const filtered = formik.values.certificate_file.filter(
      (file, index) => file.name !== fileClicked.name && indexClicked !== index
    );
    formik.setFieldValue("certificate_file", filtered);
  };
  return (
    <div
      className={
        ...
        " " +
        `${formik.values.certificate_file.length > 0 ? " pb-3  " : "  "}`
      }
    >
      <div className=" flex flex-col items-start gap-2  ">
        <div
          className={
            ...
            " " +
            `${
              formik.values.certificate_file.length > 0
                ? " after:w-[100%]  "
                : "  "
            }`
          }
        >
          ...
          >
            <input
              ...
              id="fileInputPictureCertificate"
          ...
          <div
            className={
              ...
              `${
                ...
                formik.touched.certificate_file
                  ? " opacity-100 "
                  : " opacity-0 "
              }`
            }
        ...
        {formik.values.certificate_file.map((item, index) => (
          <div
            key={index}
